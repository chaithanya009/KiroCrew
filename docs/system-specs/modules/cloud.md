...
`transitEncryption` and IAM authorization are not caller fields.
`transitEncryption` is forced ENABLED because the AWS default is DISABLED and the
traffic is the crew's transcripts. `authorizationConfig` appears only with an
access point, and then carries `iam: ENABLED`: that pairing makes the mount a
crew-scoped operation, evaluated against the per-crew task role, so a task cannot
mount an access point its own role is not permitted to.
...
Two obligations this leaves elsewhere, named so they are not discovered at first
launch. Without an access point the file system's own root must already be
writable by the container's non-root user, which belongs to whatever creates the
file system. With one, the per-crew task role needs
`elasticfilesystem:ClientMount` and `ClientWrite` for that file system, which
belongs to whatever creates the roles. And `store` is `None` on every launch the
engine builds today: there is no configuration home for a file system id yet, so
the engine states the ephemeral answer explicitly rather than inventing an id.
