...
- **Never chatted → removed** (`remove_never_chatted`): the row is deleted
  ...
  the description the row was judged with, the bound spec — re-read from disk
  under `agents_spec_lock`, nested inside the config lock in the order every
  other spec writer keeps — still carries that same description (a hand edit
  of the spec file while the pass ran, or a spec that vanished or stopped
  reading as a spec, is newer evidence), AND `config.local.json` still does
  not name it — read under the overlay's own sidecar lock, taken inside the
  base lock (base-then-overlay, the order the binding writers keep) and held
  until the base write has committed, so no overlay leaf can land for the
  name between that check and the delete. The fence is identity and shape, not equality with a default-filled
...
- **Removal needs a complete history; the pass always finishes.** Two kinds
  ...
  and the files under `unreadable_sessions`. The same holds per candidate for
