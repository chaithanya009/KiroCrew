...
the data home's `autonudge-trust/` directory, written by `autonudge_selfarm.py`;
a gateway-only leaf of its own, on `security._CREW_SECRET_LEAVES` (agent file
tools can neither read nor write it) AND bind-masked by
`sandbox._CREW_HIDDEN_LEAVES` (precreated, alias-refused), so a sandboxed
command that composes the path at runtime cannot write it either — deliberately
...
registry; the side panel host polls nothing of its own, its block already
re-renders from the pushed `wake` projection) — arms the member's own thread
...
and then land a finite cap on the owner's Perpetual mode. Other modes take
...
exactly the loop id and token it captured. ON on a STOPPED
...
no entry stands over a loop that never existed; a structured monitor is refused 409, never converted. Inside
...
while still scheduled to stop at its cap, and ON pressed over it changes
nothing (the switch never rewrites a live loop the member is driving — a
takeover is only offered over a STOPPED one, where the member has nothing in
flight to lose). The detail page's ON readout carries the loop's own wake
