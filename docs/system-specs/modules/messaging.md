...
- **`apply_mode(mode, session_key, *, source, caller, resources, sessions, notify,
  ...
  applied. The durable map row is admitted first -- it is what can refuse -- then
  the in-memory mark lands, both before any await, so a concurrent inbound
  message cannot observe the session as unrestricted after the user asked for
  privacy; then, still before any await, the audit (`f"{source}.{mode}_mode"`) —
  so a task cancelled while the awaited transcript header write is in flight (a
  gateway shutdown right after the modifier) has already written the audit
  record, which the idempotency return would never come back to write; then the
  header write, the caller's `on_applied` hook, and the notice. A persist
  FAILURE — either record — is logged, not raised — the mark already holds for
  this process, and refusing the modifier would tell the user privacy is off
  while it is on. A persist REFUSAL is different: when the map declines to
  retain the row (`SessionMap.PRIVACY_ROW_CAP` reached, or a key over
  `PRIVACY_ROW_KEY_MAX`), `apply_mode` writes one SEL `denied` record
  (`private_session_refused:<reason>:<target>`), posts `refusal_notice` -- the
  mode was not applied, the message was NOT processed, nothing ran and nothing
  was saved -- and raises `PrivacyModeRefused` with nothing marked or written.
  The caller must not run the turn: the Slack applier answers
  `only_modifier=True` (the contract both Slack callers already honour by
  returning), the Telegram command and turn paths return, and the Telegram
  steer path RESERVES before it steers. Refusing is the fail-closed answer; running the
  message with the mode silently dropped would be the leak the modifier exists
  to prevent, and evicting a retained row to make room would run THAT thread as
...
- **`reserve(mode, session_key, …) -> Reservation`**, **`commit(reservation)`**,
  **`release(reservation, *, sessions, source, …)`** — the form for a caller about
  to take a step it cannot take back. `reserve` is `apply_mode(must_persist=True)`:
  row, mark, audit, header, hook and notice before the step, the row DURABLE
  before it returns — the map's debounced write is awaited (`aflush`) right after
  the mark, under the mark's cover, and a write the map cannot perform, at
  `set_flag` or at that flush, is a refusal (`persist_failed`, the flush failure
  taking the mark and the flag back with it) rather than the best-effort mark,
  because the step would run on the strength of the row (the Telegram steer folds
  the message into a turn that persists its transcript). The shared holder for a
  (mode, key) is registered BEFORE `reserve`'s first await, so a concurrent second
  caller always finds it, joins it and waits for the first application to settle
  instead of running its own — no window in which two callers each hold "the only"
  reservation and the loser's release erases the winner's committed mode; a
  failed first application unwinds its holder and the joiner attempts its own. A
  landed step `commit`s; a step that did not land `release`s: the tracker mark is
  dropped, the map flag cleared (the row stops counting against the cap) and the
  transcript header restored to what it recorded before, only if it still records
  what the stamp wrote, with one SEL `released` record beside the `allowed` one.
  Loosening is otherwise never done, so a release checks three things first —
  refcounted per (mode, key): only the LAST pending holder releases, never when any
  holder committed, and never when the group did not newly apply the mode (a
  session already private is left exactly as it was). `reset()` drops pending
  reservations with the trackers.
