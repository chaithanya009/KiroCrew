...
materialising (`_register_agents`), pruning (`_prune_stale_app_resources`) or
removing (`_deregister_agents`) app specs, and before registering
(`_register_mcp_servers`) or deregistering (`_deregister_mcp_servers`) an app's
MCP servers into the same specs, bridges asks
...
(`_register_agents` logs one WARNING and returns `[]`; `_register_mcp_servers`
returns `[]` and `_deregister_mcp_servers` returns `0` without opening the
config; the pruner and remover return without touching the directory) and the
refusal is SEL-audited exactly as `agent._decline_shared_agent_home` audits its
own: one `api_access` row, `operation=agent_home_write`, `outcome=denied`,
`source` naming the writer that stood down (`register_agents`,
`deregister_agents`, `prune_stale_app_resources`, `register_mcp_servers`,
`deregister_mcp_servers`), `resources` the shared directory. A permission
decision about the machine-wide agent home is reconstructible from the audit log
alone, whichever of the five writers made it.
