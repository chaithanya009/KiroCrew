...
already-relocated install wrote under the host `~/.kiro/sessions/cli`, those
`<sid>.json` / `<sid>.jsonl` pairs migrate into the adopted directory LAZILY,
one session at a time, when that session is next opened. Nothing scans or moves
transcripts at startup: the gateway's initial `prune()` and every guarded
`SessionMap.get()` treat a matching host-side pair as a live transcript
(`session_map.host_transcript_pending`, one `lstat`), and the one path that hands
a sid to kiro-cli — `session_allocation._get_or_create_impl` via
`session_map.resolve_resume_sid` — runs the move on a worker thread through
`resolve_host_transcript`, which attempts the copy and decides serve/withhold
atomically under a per-sid lock. The move opens the adopted transcript directory
once through `pinned_fs.create_and_open_dir_pinned`, publishes journal first and
state file last relative to that pinned descriptor, and retires the host pair
only once both adopted files and the destination directory are durable. Recovery
is clear-and-republish: a plain residue that is a strict prefix of, or byte-equal
to, its host counterpart is deleted and the complete pair is republished from
the host pair; a longer or divergent residue holds bytes the host pair does not,
so it is left in place and the open is withheld for manual reconciliation. A
failed move leaves the host pair and the mapping intact for the next open to
retry; a failure that left residue beside the host pair withholds the sid for
that open (`ResumeLookup.withheld`), and the allocation then serves a fresh
session WITHOUT promoting its sid over the withheld mapping (see
[session](session.md#session-resume-sessionmap)). Only sids in this instance's
map move; the host directory is shared with the default instance and is otherwise
untouched. The gate canonicalizes `KIRO_HOME` and the isolated home with
`Path.resolve()` before comparing them, so equivalent spellings such as a
trailing separator agree with doctor's adopted-home predicate; the opt-out, a
pod, the CI harness and every default-home install return before touching the
host directory. The host kiro-cli MCP registry (`~/.kiro/settings/mcp.json`,
