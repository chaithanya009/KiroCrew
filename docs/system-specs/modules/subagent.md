...
serialized by the per-agent lock (`_STATE_LOCKS`). An on-loop caller cannot wait
...
site carries its own fence instead:
...
| `bind_session_execution` → `update_execution_context` | the callee holds the per-agent lock unconditionally |
| `create_agent_folder` | creation path; no writer for the agent exists yet |
