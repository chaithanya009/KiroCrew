...
`"deepseek"`, and `"custom"`; public-build selectability is a separate registry. The complete
launch and host-capability table is in
[agent-host-contract.md](agent-host-contract.md#column-meaning). Three launch paths
...
- `"custom"` (`ACP_BACKEND_CUSTOM`): the operator's own executable and argv, resolved fresh per spawn by `agent_sdk/custom_acp.py:resolve_custom_acp` from `agent.custom_acp` (`{command, args}`). The command is a PATH name or an absolute path, searched on the augmented PATH via `shutil.which`; nothing is shell-parsed and the pair is validated but never introspected. It takes the per-session `AcpClient` path — `"custom"` is in no `ACP_BACKENDS_*` capability set, so it is not runtime-multiplexed, not session-shared, and reads no `cli.json` overlay — and speaks standard numeric ACP v1. Its routing is `Routing.UNVERIFIED`, so nothing establishes that its tool calls reach the PreToolUse gate; it is admitted only behind the OS credential mask that `agent_sdk/tool_gate.py:_requires_credential_mask` returns true for (custom OR enforced), and `enforce_sandbox_floor` refuses the session where that mask cannot apply. Crew projects no agent spec and no MCP servers to it, seeds no permission setting, and loads no gate extension. See [agent-host-contract.md](agent-host-contract.md#custom-acp-experiment).
