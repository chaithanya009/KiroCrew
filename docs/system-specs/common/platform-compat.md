...
| Open one component of a path you are still walking, whatever it turns out to be | `open_entry_no_follow(path)`, then classify the DESCRIPTOR with `is_reparse_point_fd(fd)` / `os.fstat` (then `os.close`) | `pin_directory` or `open_file_no_reparse` (each refuses what it is not looking for, and a walk needs "a link sits here, read its target", "this is a file, the path ends" and "carry on" kept apart -- both answer `NotADirectoryError` for the first two); or `is_link_or_junction(path)` after the open (a second look by name, at a name that can change between the two) |
