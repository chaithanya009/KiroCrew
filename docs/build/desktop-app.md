...
kiro-cli into the app's resources at `backend-dist/kiro-cli/`. The staged
payload is the single `kiro-cli-chat` binary, not upstream's layout: the
`kiro-cli` launcher resolves `kiro-cli-chat` through `$HOME/.local/bin` and
...
subcommand the app uses; `kiro_cli.BUNDLED_KIRO_CLI_ENTRY` names it. The
release is pinned in two files that travel together: `packaging/kiro-cli-version`
names the version and `packaging/kiro-cli-sha256` holds the sha256 of each
artifact the build stages (the universal macOS DMG, the two Linux gnu zips) in
...
(~160 MB). Downloads are cached per user under `~/.cache/kirocrew-build/kiro-cli`,
...
session all run the bundled copy. Beside the directory, the shell sets
...
Windows only (the upstream chat-cli crate's `cli/mod.rs` at v2.24.0 gates the
whole block on `target_os = "windows"`; upstream's auto-update guide documents
the variable), and Windows never bundles, so today nothing in the tree self-updates: the switch
...
app's built-in kiro-cli (`bundled_cli` in the status payload), and refuses the
