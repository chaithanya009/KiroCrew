...
class TestMissingEvidenceIsABlockNotAConcern:
    ...
    def test_design_lane_reads_evidence_presence_off_a_fetched_list_not_the_description(self, name):
        ...
        if name == "fork-design-review.yml":
            ...
            # The same-repo lane also lists the committed media present at HEAD,
            # and admits a path only once the checkout proves it is a regular
            # file whose BYTES are an allowlisted media type. An extension plus
            # existence is not enough: the reviewer is told this list means the
            # evidence rendered, so a text file under an image name must not
            # reach it.
            assert 'if [ ! -f "$path" ] || [ -L "$path" ]; then' in raw
            assert 'mime="$(file --mime-type -b -- "$path")"' in raw
            assert "SKIPPED (mime $mime)" in raw
            assert (
                "committed images or recordings this revision adds or changes, "
                "typed by their bytes: $k" in raw
            )
