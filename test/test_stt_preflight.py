...
class TestProbeNative:
    ...
    def test_the_child_cannot_be_shadowed_or_fed_by_the_gateway_environment(
        ...
        assert seen["cmd"][:3] == [sys.executable, "-I", "-c"]
        assert seen["cmd"][4:] == ["/site/_pywhispercpp.so"]
...
class TestAChildThatCannotStart:
    def test_a_missing_interpreter_is_inconclusive_not_a_raise(self, monkeypatch) -> None:
        ...
        assert code is None
        assert "python-that-is-gone" in err or "No such file" in err
...
class TestLoadMarker:
    ...
    def test_a_marker_from_a_binary_that_is_gone_is_cleared(
    ...
    def test_garbage_is_cleared_not_trusted(self, tmp_path: Path, ident) -> None:
        (tmp_path / preflight.LOAD_MARKER_NAME).write_text("not json", encoding="utf-8")
        assert preflight.previous_load_crashed(tmp_path).ok is True
        assert not (tmp_path / preflight.LOAD_MARKER_NAME).exists()
