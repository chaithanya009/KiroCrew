...
def test_no_access_point_is_a_valid_store_rather_than_a_refusal():
    ...
    would refuse that design rather than a mistake.
    """
    store = td.StoreSpec(file_system_id=LONG_FS)
    assert store.access_point_id == ""
    assert "authorizationConfig" not in efs(td.task_definition_document(spec(store)))
...
def test_an_access_point_brings_iam_authorization_with_it():
    """The pair is what scopes a mount to the crew's own task role."""
