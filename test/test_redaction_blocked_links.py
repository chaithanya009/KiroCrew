...
class TestRecordsReachTheDisplayPath:
    ...
    def test_an_interrupted_reply_collects_records_at_its_own_redaction(self) -> None:
        ...
        for ln in lines[start + 1 :]:
            ...
            body.append(ln)
        src = "\n".join(body)
        assert "redact_exfiltration_urls_with_records(body)" in src
        assert "_segment_row_meta(slot, _blocked)" in src
...
class TestEveryRetentionPointIsBounded:
    ...
    variant list goes through ``with_bounded_blocked_links``.
    """
    ...
    def test_the_helper_bounds_and_drops_and_leaves_other_keys(self) -> None:
        from kiro_crew.dashboard.chat_utils import with_bounded_blocked_links

        bounded = with_bounded_blocked_links({"mid": "m1", "blocked_links": self._many()})
        assert bounded["mid"] == "m1"
        assert len(bounded["blocked_links"]) == exfil.MAX_BLOCKED_LINKS_PER_MESSAGE

        assert with_bounded_blocked_links({"blocked_links": [{"domain": "x"}]}) == {}
        plain = {"mid": "m1"}
        assert with_bounded_blocked_links(plain) is plain
    ...
    def test_each_retention_site_uses_the_bounded_helper(self) -> None:
        ...
        assert persistence.count('with_bounded_blocked_links(m["meta"])') == 2
        assert "with_bounded_blocked_links(" in inspect.getsource(chat_persistence._attach_variants)
        stash = inspect.getsource(chat_runner._flush_segment)
        assert "with_bounded_blocked_links({**v" in stash
