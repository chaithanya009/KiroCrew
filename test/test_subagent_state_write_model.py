...
ESCAPE HATCH. A flagged line may carry a trailing ``# state-write-ok: <reason>``
comment. The reason is mandatory so the exception is auditable in review and
greppable later. Prefer a census entry, which records the same reason where the
policy lives.
"""
...
_WRITE_SITES: dict[tuple[str, str, str], tuple[int, str]] = {
    ...
    ): (1, "on loop; fenced by the callee's unconditional per-agent lock"),
    (
    ...
    (
        ...
        "create_agent_folder",
        "update_execution_context",
    ): (1, "creation path; no concurrent writer exists for the agent yet"),
    (
        "kiro_crew/subagent_persistence.py",
        "create_agent_folder",
    ...
    (
        ...
        "update_execution_context",
    ): (1, "fenced by the callee's unconditional per-agent lock"),
    (
        "kiro_crew/subagent_persistence.py",
        "update_execution_context",
        "_atomic_write(state.json)",
    ...
    (
        ...
        "write_run_agent",
        "update_execution_context",
    ): (1, "pre-run write; fenced by the callee's unconditional per-agent lock"),
}
...
def _suppressed_lines(source: str) -> set[int]:
    """Lines carrying a trailing ``# state-write-ok`` comment.

    Tokenized rather than substring-matched, so the marker inside a string
    literal -- this module's own docstring, for one -- suppresses nothing.
    """
    out: set[int] = set()
    try:
...
class _SiteScan(ast.NodeVisitor):
    ...
    def __init__(self, path: str, aliases: dict[str, str], suppressed: set[int]) -> None:
        self.path = path
    ...
    def visit_Call(self, node: ast.Call) -> None:
        ...
        if label is not None:
            ...
            if not any(line in self.suppressed for line in span):
                self.hits.append(
                    (
                        self.path,
...
def find_write_sites(
    ...
    *tree* lets a caller that already parsed *source* hand the tree over; it must
    be the parse of *source*, since the suppression scan re-tokenizes the text.
    """
    if tree is None:
        tree = ast.parse(source)
    scan = _SiteScan(path, _import_aliases(tree), _suppressed_lines(source))
    scan.visit(tree)
    return scan.hits
...
def test_no_new_whole_file_state_write_site() -> None:
    ...
    if problems:
        raise AssertionError(
            ...
            "count and the fence named, or mark the line "
            "'# state-write-ok: <reason>'.\n" + "\n".join(problems)
        )
...
def test_a_suppression_comment_suppresses_one_line() -> None:
    src = "def f(a):\n    update_state(a, keep=False)  # state-write-ok: fenced by a caller check\n"
    assert find_write_sites(src) == []


def test_the_marker_inside_a_string_does_not_suppress() -> None:
    src = 'def f(a):\n    note = "state-write-ok"\n    update_state(a, keep=False)\n'
    assert _labels(src) == ["update_state"]
