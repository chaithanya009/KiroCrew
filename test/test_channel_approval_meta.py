...
@pytest.mark.asyncio
async def test_every_retained_field_is_bounded_and_an_over_limit_command_is_display_only():
    """The message ring is capped by count; each retained string is capped by
    the same named bound, so a model-authored command cannot grow persisted
    history. A command past the bound is shown elided, with a visible marker,
    and offers no per-command tier: its title could only ever be a prefix of
    the command the endpoint would compare against."""
    long_cmd = "echo " + "x" * (_APPROVAL_FIELD_MAX_CHARS + 200)
    args, kw = await _approval_post(long_cmd)
    meta = kw["meta"]
    assert all(len(v) <= _APPROVAL_FIELD_MAX_CHARS for v in meta.values())
    assert len(meta["tool_title"]) == _APPROVAL_FIELD_MAX_CHARS
    assert meta["tool_title"].endswith("…")
    assert meta["is_shell"] == "1"
    assert meta["command_grantable"] == ""
    assert meta["base_derivable"] == ""
    assert meta["base_command"] == ""
    # The prose carries the same bounded, marked title; nothing longer.
    assert meta["tool_title"] in args[1]
    assert "x" * (_APPROVAL_FIELD_MAX_CHARS + 1) not in args[1]


@pytest.mark.asyncio
async def test_grantable_title_is_never_elided_at_the_bound():
    """The bound is measured on the TITLE (prefix + command), not the command:
    the longest grantable command still renders whole, and one character more
    flips to display-only rather than to a silently cut title -- the reader
    must never approve a prefix while the full command runs."""
    room = _APPROVAL_FIELD_MAX_CHARS - len(_APPROVAL_SHELL_TITLE_PREFIX)
    fits = "echo " + "y" * (room - len("echo "))
    assert len(fits) == room
    _args, kw = await _approval_post(fits)
    assert kw["meta"]["command_grantable"] == "1"
    assert kw["meta"]["tool_title"] == f"{_APPROVAL_SHELL_TITLE_PREFIX}{fits}"
    assert kw["meta"]["base_command"] == "echo"

    _args, kw = await _approval_post(fits + "y")
    assert kw["meta"]["command_grantable"] == ""
    assert kw["meta"]["base_derivable"] == ""
    assert not kw["meta"]["tool_title"].startswith(_APPROVAL_SHELL_TITLE_PREFIX)
    assert len(kw["meta"]["tool_title"]) <= _APPROVAL_FIELD_MAX_CHARS
...
async def test_post_threads_meta_and_defaults_to_none():
    ...
    facts = {"is_shell": "1", "base_command": "ls"}
...
def test_serialize_round_trips_meta_and_tolerates_its_absence():
    ...
    with_meta = ChannelMessage(
        ...
        content="card",
        msg_type="approval",
        meta={"is_shell": "1", "base_command": "ls"},
    )
    ...
    assert by_id["m1"].meta == {"is_shell": "1", "base_command": "ls"}
