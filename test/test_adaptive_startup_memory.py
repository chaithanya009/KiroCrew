...
def test_reaper_sweep_publishes_the_learned_cost_off_loop(monkeypatch, tmp_path) -> None:
    ...
    try:
        ...
        monkeypatch.setattr(subagent_mod, "read_learned_costs", MagicMock(side_effect=OSError))
        mgr._sample_live_costs()
        assert mgr._learned_costs_gb == expected
        # A read that DEGRADES to nothing while the log is still present keeps
        # them too (read_learned_costs never raises on an unreadable file).
        monkeypatch.setattr(subagent_mod, "read_learned_costs", lambda *a, **k: {})
        mgr._sample_live_costs()
        assert mgr._learned_costs_gb == expected
        # A PARTIAL read (one bucket lost to a corrupt line or a truncating
        # over-cap record) keeps the missing bucket and takes the new figure
        # for the one it did yield -- up or down.
        monkeypatch.setattr(subagent_mod, "read_learned_costs", lambda *a, **k: {"light": 0.7})
        mgr._sample_live_costs()
        assert mgr._learned_costs_gb == {
            "kirocrew": pytest.approx(6.0),
...
def test_expired_samples_do_not_price_a_start(monkeypatch, tmp_path) -> None:
    ...
    assert sc.read_learned_costs("mem_gb") == {"kirocrew": pytest.approx(6.0)}
...
def test_the_held_map_is_bounded_against_an_agent_writable_log(monkeypatch, tmp_path) -> None:
    ...
    assert min(costs.values()) > 0.1 + 9 * 0.01  # the lightest ten were the ones dropped
...
def test_a_log_replaced_during_the_read_keeps_the_prior_map(monkeypatch, tmp_path) -> None:
    ...
    try:
        mgr._sample_live_costs()
        assert mgr._learned_costs_gb == {"heavy": pytest.approx(6.0)}
        real_read = subagent_mod.read_learned_costs
        ...
        monkeypatch.setattr(subagent_mod, "read_learned_costs", read_then_replace)
        mgr._sample_live_costs()
        # Pre-reset figures were NOT paired with the replacement file.
        assert mgr._learned_costs_gb == {"heavy": pytest.approx(6.0)}
        monkeypatch.setattr(subagent_mod, "read_learned_costs", real_read)
        mgr._sample_live_costs()
