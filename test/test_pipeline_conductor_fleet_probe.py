...
def _can_create_symlink() -> bool:
    """Whether this host lets a test create a symlink, measured once.

    Reading the capability rather than the platform name is what keeps the skip
    honest in both directions: a Windows host that grants the privilege runs
    these cases, and a host that does not skips them for the reason that is
    actually true of it.
    """
...
# The ONLY capability that cannot be stubbed here. ``os.sysconf`` and
# ``os.getloadavg`` are absent on Windows, which the probe's own source already
# treats as optional -- the age is uncomputable without a clock-tick rate and the
# load average sits behind a ``hasattr`` guard -- so the cases that need them
# patch them in with ``raising=False`` and run everywhere, and the answer a host
# without a clock source gets is asserted on every platform. Creating a link is
# different: it is a privilege, not an attribute, so it can only be measured.
needs_symlinks = pytest.mark.skipif(
    not _can_create_symlink(), reason="this host does not permit creating a symlink"
)


@pytest.fixture
def mod():
    return load_skill_script("fleet_probe", SCRIPT)
...
def test_owner_class_reads_the_working_directory(mod, tmp_path):
    ...
    entry = tmp_path / "201"
    entry.mkdir()
    (entry / "cwd").symlink_to(fleet / "src")
...
@needs_symlinks
def test_an_unresolvable_root_reads_as_unknown_not_as_fleet(mod, tmp_path):
    """A root that cannot be compared must widen nothing."""
    entry = tmp_path / "204"
    entry.mkdir()
    (entry / "cwd").symlink_to(tmp_path)
    unresolvable = "/a\0b"
    assert mod._owner_class(entry, [unresolvable]) == "unknown"
    assert mod._program_class("/usr/bin/python3 -m pytest", [unresolvable]) == "unknown"
...
def test_host_lines_counts_someone_elses_run_without_printing_it(mod, tmp_path, monkeypatch):
    ...
    (entry / "cwd").symlink_to(elsewhere)
    lines, host = mod._host_lines({"fleet_worktrees": [str(fleet)]})
    assert lines == []
