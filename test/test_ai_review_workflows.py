...
class TestUxReviewReadsTheScreenshotsBlindFirst:
    ...
    def _run_evidence_gate(
        ...
        out = subprocess.run(
            ...
            env={
                **env,
                "BASE_SHA": base,
                # The fork lane also reads the media the PR commits, out of the
                # object store at its head; the same-repo step, which has the
                # files checked out, ignores this.
                "HEAD_SHA": "HEAD",
                "REPO": "example/repo",
                "PR": "7",
    ...
    def test_same_repo_design_lane_lists_only_regular_byte_typed_media(self) -> None:
        """The same-repo design lane treats a committed path as rendered evidence
        only after the checkout proves it is a regular allowlisted media type.
        A matching extension alone cannot admit text or a tracked symlink."""
        script = _step_script(_workflow("design-review.yml"), "Collect rendered evidence")
        assert 'git cat-file -e "HEAD:$path"' not in script
        assert 'if [ ! -f "$path" ] || [ -L "$path" ]; then' in script
        assert "SKIPPED (not a regular file at HEAD)" in script
        assert 'mime="$(file --mime-type -b -- "$path")"' in script
        for mime in (
            "image/png",
            "image/jpeg",
            "image/webp",
            ...
            assert mime in script
        assert "SKIPPED (mime $mime)" in script
        assert 'echo "committed images or recordings:"' in script
        assert script.index('case "$mime" in') < script.index('committed="${committed}${path}"')
        assert "typed by their bytes" in script
