...
class TestPrivateConversationCap:
    ...
    @pytest.mark.asyncio
    async def test_a_row_that_cannot_be_written_refuses_when_the_row_is_required(
        self, audits, session_map, monkeypatch
    ):
        """``must_persist``: the reservation the steer path takes before an
        irreversible step. A map that cannot write the row is a refusal
        (``persist_failed``) -- no mark, one denial, the user told -- where the
        best-effort form would have marked the session for this process only.
        Mutation: ignore ``must_persist`` -- the mode applies with no row."""
        sm = session_map()
    ...
    @pytest.mark.asyncio
    async def test_a_row_that_cannot_be_written_still_marks_when_the_row_is_best_effort(
        self, audits, session_map, monkeypatch
    ):
        """The default form keeps its contract: the mark holds for this process,
        the user is told the mode is on, the failure is logged rather than raised."""
        sm = session_map()
        monkeypatch.setattr(sm, "set_flag", MagicMock(side_effect=OSError("disk full")))
        rec = _Recorder()
        applied = await privacy_mode.apply_mode(
...
class TestReservations:
    ...
    @pytest.mark.asyncio
    async def test_a_strict_row_is_on_disk_before_apply_mode_returns(self, audits, session_map):
        """``must_persist`` means DURABLE: the map's debounced write is awaited, so a
        fresh map reads the row back with no flush of the test's own. Mutation: skip
        the ``aflush`` -- the fresh map reads no flag."""
        sm = session_map()
        applied = await privacy_mode.apply_mode(
            privacy_mode.MODE_INCOGNITO,
            _TG_KEY,
    ...
    @pytest.mark.asyncio
    async def test_a_failed_flush_refuses_and_takes_the_mark_back(
        self, audits, session_map, monkeypatch
    ):
        """The row reached the map's memory but not the disk: with ``must_persist``
        that is a refusal -- the mark is taken back, the flag cleared, one denial,
        the user told -- where the best-effort form would have marked and carried
        on. Mutation: do not await the flush -- the mode applies."""
        sm = session_map()
        ...
        assert rec.notices == [privacy_mode.refusal_notice("incognito", "persist_failed")]
        assert [e["outcome"] for e in audits] == ["denied"]
        # The best-effort form is untouched by a failing flush: it never awaits it.
        applied = await privacy_mode.apply_mode(
            privacy_mode.MODE_INCOGNITO, _TG_KEY, source="telegram", sessions=_Sessions(sm)
        )
        assert applied is True and privacy_mode.is_incognito(_TG_KEY)
