...
def test_the_anchor_lookup_is_bounded_and_an_older_dispatch_leaves_it_unthreaded(monkeypatch):
    """The bound is the point: an unbounded scan per report grows with the history.

    A dispatch beyond the window leaves the report unthreaded, which the spec
    allows, rather than costing a full read of the crew's whole log.
    """
