...
BENIGN_SPAWNS: frozenset[str] = frozenset(
    {
        ...
        # The speech-runtime preflight child: `sys.executable -I -c <module-level
        # string constant> <extension path>`, no shell, a bounded timeout, cwd
        # pinned to the interpreter prefix and `-I` keeping the working directory
        # off `sys.path` so nothing the gateway was started from can shadow the
        # import, and an environment reduced to a fixed allow-list (no credentials,
