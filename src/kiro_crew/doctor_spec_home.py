...
_DriftRank = tuple[int, str, int]
_RankedDrift = tuple[_DriftRank, HomeDrift]


def _retain_drift(retained: list[_RankedDrift], *, rank: _DriftRank, drift: HomeDrift) -> None:
    """Keep only the first report window under the rendered ordering contract."""
    if len(retained) < DRIFT_REPORT_MAX_ITEMS:
        retained.append((rank, drift))
        return
    worst = max(range(len(retained)), key=lambda index: retained[index][0])
    if rank < retained[worst][0]:
        retained[worst] = (rank, drift)


class _AgentSpecScanError(Exception):
    """The agents directory became unreadable while its entries were streamed."""


def _iter_spec_paths(agents_dir: Path) -> Iterator[Path]:
    try:
        ...
        raise _AgentSpecScanError(str(exc)) from exc
...
def check_spec_home_drift(*, agents_dir: Path | None = None) -> HomeDriftReport:
    ...
    retained: list[_RankedDrift] = []
    managed_drift_seen = 0
    foreign_drift_seen = 0
    try:
        for spec_path in _iter_spec_paths(agents_dir):
            # The one hardened reader every spec consumer uses: refuses a
            # symlink whose resolved target is sensitive, a non-UTF-8 or
            # oversized file, and anything that is not a JSON object -- all
            # as ``None``, fail-open per file. The agents dir is user-writable
            # and shared with other tools, so a bare ``read_text`` here would
            # be the one spec read outside the gate.
            ...
            if not isinstance(servers, dict):
                continue
            for server_index, (server, entry) in enumerate(servers.items()):
                if not isinstance(entry, dict):
                    continue
                server_name = str(server)
                managed = (
                    spec_path.name in managed_names and server_name in MANAGED_MCP_SERVER_NAMES
                )
                ...
                if not pinned or _lexical(pinned) in expected_forms:
                    continue
                if managed:
                    managed_drift_seen += 1
                else:
                    foreign_drift_seen += 1
                drift = HomeDrift(
                    spec=_bounded_drift_field(spec_path.name),
                    server=_bounded_drift_field(server_name),
                    pinned=_bounded_drift_field(pinned),
                    # "Managed" -- rewritable by ``setup --agent-only`` --
                    # needs BOTH an owned spec file AND one of Kiro Crew's
                    # own server names. A user-added custom server inside
                    # kirocrew.json is preserved by the rebuild, so a drifted
                    # pin there would survive the remedy doctor recommends;
                    # it is reported as foreign (edit by hand).
                    managed=managed,
                )
                # This rank is the old rendered order: managed rows first,
                # then spec filename, then JSON object order within a spec.
                # The bounded selector keeps the lexical first cap-window
                # without retaining every path or every drift row.
                rank = (0 if managed else 1, spec_path.name, server_index)
                _retain_drift(retained, rank=rank, drift=drift)
    except _AgentSpecScanError as exc:  # pragma: no cover - defensive: dir vanished mid-scan
        logger.debug("agents dir %s unreadable: %s", agents_dir, exc)
        return HomeDriftReport(expected=str(home))

    retained.sort(key=lambda item: item[0])
    report.drift = [drift for _, drift in retained]
    retained_managed = sum(drift.managed for drift in report.drift)
    report.managed_omitted = managed_drift_seen - retained_managed
    report.foreign_omitted = foreign_drift_seen - (len(report.drift) - retained_managed)
    return report
