...
def _startup_cost_gb(agent: Any, learned_gb: float | None) -> float:
    ...
    *learned_gb*, the per-agent p90 the cap is already sized from
    (:func:`~kiro_crew.subagent_cost.read_learned_cost`, refreshed off-loop by
    the reaper sweep and held on the manager, so this is arithmetic only). The
...
class SubagentManager:
    ...
    def __init__(
        ...
        # (``learned_cost_for`` → ``_startup_cost_gb``; a bucket with no history
        # is priced at the heaviest known). Empty until the first off-loop
