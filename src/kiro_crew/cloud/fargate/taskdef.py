...
class StoreSpec:
    ...
    ``transit_encryption`` and ``iam`` authorization are NOT fields. Transit
    encryption is forced on: the traffic is the crew's transcripts, and the AWS
    default is off, so a field here would make the safe value a thing to remember.
    IAM authorization is forced on wherever an access point is used, because the
    pair is what scopes the mount to the crew's own task role; see
    :func:`_store_volume`.
    """
...
def _store_volume(store: StoreSpec) -> dict[str, Any]:
    ...
    Transit encryption is set here rather than accepted, and the reason is the AWS
    default: ``transitEncryption`` defaults to DISABLED, so a document that simply
    does not mention it mounts the crew's transcripts over an unencrypted NFS
    connection. A field for it would make the safe value something a caller has to
    remember, and every caller is this module.

    ``authorizationConfig`` appears only with an access point, and then carries
    ``iam`` ENABLED. That pairing is what makes the mount a crew-scoped operation:
    authorization is evaluated against the task role, which is derived per crew, so
    a task cannot mount an access point its own role is not permitted to. Without an
    access point there is nothing to scope and no role condition to state, so the
    ...
    if store.access_point_id:
        configuration["authorizationConfig"] = {
            "accessPointId": store.access_point_id,
            "iam": "ENABLED",
        }
    return {"name": CREW_STORE_VOLUME_NAME, "efsVolumeConfiguration": configuration}
