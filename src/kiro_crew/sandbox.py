...
_CREW_HIDDEN_LEAVES: tuple[str, ...] = (
    ...
    # given NO window, which is the difference from ``scratch`` above: every
    # consumer runs unsandboxed (the gateway's own maintenance sweep, operator
    # script crons, an app backend), so no sandboxed process reaches it and no
    # environment variable names it. Keys are deterministic by design -- an
...
_CREW_PRECREATE_HIDDEN_DIR_LEAVES: tuple[str, ...] = (
    ...
    # key it expects a job to use -- keys are deterministic by design -- and
    # ``allocate_work`` would then REJOIN the planted tree. Materialised empty at
    # 0700 before every spawn so the bind always has a target.
    "work",
)
