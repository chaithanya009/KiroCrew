...
def _warn_backend_not_stopped(app_name: str) -> None:
    ...
    print(
        ...
        "the CLI can do from out here. If one is still running it holds its port "
        "until the next gateway start, which terminates backends left behind by a "
        "previous generation. To stop it now, start the gateway.",
        file=sys.stderr,
    )
