...
def test_an_entry_removed_after_it_is_listed_is_not_a_failure(tmp_path, monkeypatch):
    ...
    def _open_after_deleting(target: Path):
        if target == path:
            path.unlink()
...
def test_the_authority_files_are_still_uploaded_when_a_transcript_is_refused(tmp_path):
    """A partial cycle leaves the index for what is in the bucket, and says it failed."""
    settings = _settings(tmp_path)
    ...
    for name in keys.AUTHORITY_NAMES:
        assert keys.authority_key(settings, name) in store.objects
...
def test_a_directory_named_like_a_transcript_is_not_in_the_set(tmp_path):
    ...
    names = [key for key, _ in backup_mod.objects_to_back_up(settings)]
...
def test_a_file_without_the_transcript_suffix_is_not_in_the_set(tmp_path):
    ...
    paths = [path.name for _, path in backup_mod.objects_to_back_up(settings)]
