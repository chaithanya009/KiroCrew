...
def test_the_authority_files_are_uploaded_before_the_transcripts_they_index(tmp_path, bucket):
    first = _first_task(tmp_path)
    _write_transcript(first, STEM, b"turn\n")
    _write_authority(first)
    ...
    assert bucket.puts.index(transcript) > max(bucket.puts.index(k) for k in authority)
...
def test_a_transcript_that_grows_after_it_is_opened_uploads_the_length_it_had(tmp_path, bucket):
    ...
    snapshot = backup_mod.open_snapshot(path)
