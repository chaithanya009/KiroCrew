...
def sessions_prefix(settings: Settings) -> str:
    """Namespace-relative prefix under which conversation transcripts live.

    Stated as its own function because it is the prefix the owner's control plane
    reads archived segments from, so it is part of the layout contract rather than an
    implementation detail of one caller.
    """
