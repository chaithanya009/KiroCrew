...
def open_append_nolink(path: str | os.PathLike) -> int:
    ...
    * the leaf is ``lstat``-ed by :func:`_refuse_unsafe_leaf` and refused when it
      is a link or not a regular file, on every platform;
    * where the dir_fd verbs exist the leaf is opened RELATIVE to the pinned
    ...
    Where the dir_fd verbs do not exist the open stays by name, which is the
    runtime's own recorded degradation for its publish path: the ``lstat`` and
    ``fstat`` legs hold everywhere, ancestor pinning is what is lost. The
    returned descriptor is open and the caller closes it.
    """
    target = Path(path)
    refuse_linked_parents(target)
