...
#: small flat JSON document (its free-text fields are clipped at ingest), so
...
def _redact(text: str) -> str:
    """Scrub a string a record will carry: a runner's error, a caller's param.
...
def _validated_params(params: Mapping[str, str] | None) -> dict[str, str]:
    """A caller's run parameters as a flat, bounded, scrubbed map of strings.
    ...
    Values are scrubbed, keys are not. A key is a name the app author writes in
    two places -- the ``start`` call and the runner reading it back -- so
    rewriting it would break that lookup while looking like nothing happened. A
    value is data, and this channel is NOT a way to hand a runner a secret: the
    record is durable, so a credential-shaped value is scrubbed here, once,
    before either the handle or the record holds it. That keeps the value the
    runner reads and the value on disk the same string; the alternative (clean on
    disk, raw in memory) makes a record that cannot be resumed from what it says.
    """
    if params is None:
        return {}
    ...
    for key, value in params.items():
        ...
        out[key] = _redact(value)[:_MAX_PARAM_VALUE]
    return out
...
class JobRun:
    ...
    ``params``, and both are strings by the time they reach the record because
    each is checked where it enters -- so the record stays JSON-safe by
    construction and the writer is left with nothing to decide. Everything else
    the SDK mints itself.
    """
    ...
    #: ``_MAX_PARAMS`` of them, bounded and scrubbed by ``_validated_params``
...
class JobHandle:
    ...
    def params(self) -> dict[str, str]:
        """What the caller named this run's work with, already checked and clipped.
...
class JobSDK:
    ...
    def _persist(
        ...
        * the record is JSON-safe by construction: every field except ``error``
        ...
        # checks and scrubs it at the single call that accepts it, so a record
        # reaching this writer already carries strings within bounds, and
        # re-scrubbing here would only be a second place to keep one rule.
    ...
    def start(
        ...
        *,
        ...
        durable, so a credential-shaped value is scrubbed like an error is.
