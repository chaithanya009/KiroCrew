...
def surface_of(session_key: str) -> str:
    ...
    """
    head = session_key.split(":", 1)[0].strip()
    return head or "channel"
...
async def render_refusal(renderer: Any, text: str = REFUSAL_TEXT) -> None:
    """Put the refusal into a channel's OWN output stream.

    Every channel builds a renderer and finalizes it in a ``finally``, so a chunk
    dispatched here is flushed by that existing close as the turn's reply. This is
    what makes the pause visible on the surface where it happened, which is the
    half the per-message echo guards do not have: their drop is silent.
