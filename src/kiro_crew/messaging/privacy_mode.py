...
async def apply_mode(
    ...
    *must_persist* the map's write is awaited right after the mark, under the
    ...
    mark(mode, session_key)
    if must_persist and sessions is not None and (sm := conv_state_map(sessions)) is not None:
        # The row set above is in the map's memory with a DEBOUNCED write owed;
        # a caller that must have it durable before its irreversible step waits
        # for the write here. After the mark, not before: the mark is what keeps
        # a concurrent message on this session from observing it unrestricted
        # while the disk write runs, and a write that fails takes the mark back
        # with it -- so a failure leaves neither record and refuses, exactly as
        # a row the map declined would have.
        try:
            await sm.aflush()
        except Exception as exc:
...
async def release(
    ...
    *,
    ...
    the mode -- then the tracker mark is dropped, the map flag is cleared (the
    row stops counting against the cap; a row that carried another privacy
    flag keeps that one) and the transcript header is restored to what it
    recorded before, only if it still records what the stamp wrote. One SEL
    record (``released``) documents the reversal beside the ``allowed`` record
    the reservation wrote. A session that was already in the mode is left
    exactly as it was.
    """
    key = (reservation.mode, reservation.session_key)
    pending = _pending.get(key)
    ...
    if sm is not None:
        try:
            ...
            logger.warning("could not clear the %s flag of %s on release", mode, session_key)
    if sessions is not None:
        from kiro_crew.history import ConversationLog, transcript_privacy_mode
