...
class _BuiltTargets(set[str]):
    ...
    ``_anchor_both_separators``, neither of which touches the filesystem. So an
    ordinary symlinked dotfile under
    ``$HOME`` -- what a stow or chezmoi home produces -- is never resolved here and
    cannot set this flag; the two roots in that list cannot either, for the reason
    below. What remains, and what the flag therefore means, is a symlinked leaf
    under one of the three home-override roots.
...
def _report_expiry_pin(pinned: bool) -> None:
    ...
    under ``KIRO_HOME``, or a harness credential leaf under its own override --
    every build reports a traversal,
    every expiry is therefore the floor, and both knobs read as no-ops to whoever
    tunes them. The refusals then come back with nothing in the log to say why the
    fix did not apply to this host. An ordinary symlinked dotfile does NOT do it,
    and a stow or chezmoi home is therefore not the case to look for: the bulk
    targets are never resolved, so they cannot report a traversal.
