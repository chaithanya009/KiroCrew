...
#: binary inside it lives on this side, in :data:`BUNDLED_KIRO_CLI_ENTRY`, so
...
def find_kiro_cli_candidates(
    ...
    *,
    ...
    :data:`BUNDLED_KIRO_CLI_ENTRY`: the bundle ships only the chat binary, for
    ...
    candidates.extend(
        str(Path(directory) / (BUNDLED_KIRO_CLI_ENTRY if directory == bundled else name))
