...
def _run_probe_child(timeout: float, extension: str) -> tuple[int | None, str, str]:
    ...
    shim. Because ``-I`` also hides a user-site install, the child is handed the
    exact file the parent resolved (*extension*, from :func:`binary_identity`)
    ...
    """
    cmd = [sys.executable, "-I", "-c", _PROBE_SOURCE, extension]
...
def write_load_marker(models_dir: Path, model_path: str) -> None:
    ...
    Best-effort: a marker that cannot be written costs the fuse, not the load.
    """
    ident = binary_identity()
    payload = {
    ...
    try:
        ...
        atomic_write(_marker_path(models_dir), json.dumps(payload))
    except OSError:
        logger.debug("Could not write the speech load marker", exc_info=True)
...
def previous_load_crashed(models_dir: Path) -> Verdict:
    ...
    is gone; it is removed and forgotten, because the fuse is per binary. A
    marker from another process naming THIS binary is the fuse, tripped.
    """
    ...
    try:
        ...
        # Unreadable is not evidence either way; clear it so it cannot wedge.
        clear_load_marker(models_dir)
        return Verdict(True)
    if not isinstance(data, dict):
        clear_load_marker(models_dir)
        return Verdict(True)
