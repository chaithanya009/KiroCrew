...
* The *key* is caller-supplied and STABLE (an issue number, a pull-request
  number, a content fingerprint). Deterministic naming is the point, because
  a later run finds the directory again by computing the same key. Keys are
  therefore VALIDATED rather than sanitized (:data:`_KEY_RE`): rewriting a key
  would map two distinct ones onto a single directory and mix their work.
...
The root is NOT handed to sandboxed agent processes. ``sandbox`` masks it by
name beside the scratch root, and no environment variable names it: every
consumer runs unsandboxed (the gateway's own maintenance, operator script
crons, an app backend), and a variable beside ``KIROCREW_SCRATCH`` would
collapse the distinction between per-process residue and cross-process state
that this module exists to draw. An agent that needs a durable directory gets
one from the code that spawns it, not from its own environment.
...
#: Leaf of the data home holding every cross-process work directory. Named
#: here and in ``sandbox._CREW_HIDDEN_LEAVES``; a test pins the two together,
#: so a rename here cannot quietly unmask the root for sandboxed agents.
WORK_DIRNAME = "work"
...
class WorkRootBoundaryError(Exception):
    """A managed path is a link, so nothing is created or written through it."""
...
def _refresh(directory: Path) -> None:
    ...
    directory's mtime, and discarding an absent marker unlinks nothing.
    """
    try:
        os.utime(directory)
    except OSError:
        logger.debug("work-root: could not refresh %s", directory, exc_info=True)


def allocate_work(key: str) -> Path:
    """Create or REJOIN the work directory named by *key*, and return it.

    Idempotent by design: a later, different process passing the same key gets
    the same directory, which is the whole reason this root exists.
    ...
    :class:`WorkRootBoundaryError` when a managed component is a link.
    """
