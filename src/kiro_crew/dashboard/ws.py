...
async def _handle_eventlog_frame(
    ...
    if msg_type == "eventlog_unsubscribe":
        ...
        return
    # Warmed off-loop before the first grant check, because a cold lookup resolves
    # `installed.json` and the app manifest INLINE and this handler runs on the
    # serving loop, so the first subscription of an app's session would stall every
    # other task (no-blocking-call-on-event-loop). The HTTP side is warmed at the
    # auth middleware, but a frame on an established socket does not pass through
    # it, so this path needs its own.
    #
    # HERE and not before the SECOND check below: this one is already upstream of a
    # real await (`resolve_unit` runs off-loop), so one more await changes nothing,
    # and warming here makes the second check a cache hit as well. The second check
    # must stay synchronous with its registration -- its own comment explains that an
    # await inserted between them reopens the revocation race it exists to close.
    await grants.warm(ws_app)
    ...
    # Asked AGAIN, and deliberately with nothing awaitable between this check and
    # the registration below. The first check is upstream of a real await
    # (`resolve_unit` runs off-loop), so a teardown landing in that window revokes
    # the grant and closes this app's sockets while this continuation is suspended
    # -- and the close cannot see a subscription that does not exist yet, so the
    # continuation would register one the sweep already passed. Teardown revokes
    # BEFORE it closes, so a revocation that beats this check is caught here, and
    # one that loses to it is caught by the close because the subscription is
    # registered by then. Both halves are synchronous, so the loop cannot
    # interleave between them and leave a third case; an await inserted between
    # them would reopen exactly this finding.
    if not grants.may_use_kind(ws_app, kind):
        await _refuse("unit_kind_not_granted", f"this app may not subscribe to {kind!r} units")
        return
    try:
