...
async def api_eventlog_events_post(request: web.Request) -> web.Response:
    ...
    try:
        ...
        # timestamp is captured so the release below decrements the SAME UTC day
        # bucket this charge went into, rather than whichever day the clock reads
        # once the append has failed.
    ...
    def _append() -> dict:
        # Both grants the check above asked are re-asked under the write lock. The
        # handoff below is an await, so an app revoked while its append is in flight
        # has already passed that check -- the same window the publish path closes
        # with `still_granted`, and the same remedy.
        return unit.service().append(
            unit_id,
            event_type,
            data,
            still_granted=lambda: grants.may_use_kind(app, kind)
            and grants.may_append(app, kind, event_type),
        )

    try:
        event = await asyncio.to_thread(_append)
    except Exception as exc:
        # The charge above paid for an event that did not land, so it goes back. A
        # charge is a reservation taken before the write, and the write is what
        # decides: the grants are re-asked under the write lock, so an app revoked
        # while its appends are in flight has EVERY one of them refused there. Left
        # charged, those refusals spend its whole day's budget on nothing, and it
        # answers 429 once the grant returns even though the log holds none of them.
