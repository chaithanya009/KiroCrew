...
def _scrub_text(val: str) -> str:
    ...
    A CONTROL character is terminal-escape material rather than text a user wrote, so it
    always leaves: it drives a terminal that renders the field verbatim, and no consumer
    is worse off without it. Removing one can JOIN a token back together, which is why the
    redactors run again afterwards rather than trusting the first verdict.
...
def _scrub_decoded(val: object, depth: int = 0) -> object:
    ...
    An object's NAMES are scrubbed alongside its values. A name is text the document
    carries and the field egresses, so a credential sitting in one leaves untouched if
    only values are walked, and the name is the easier place to put it: whoever writes
    structured memory chooses both halves of a pair.
    ...
    if isinstance(val, str):
        return _scrub_json_transport(_scrub_text(val), depth + 1)
    if isinstance(val, list):
        return [_scrub_decoded(item, depth + 1) for item in val]
    if isinstance(val, dict):
        cleaned: dict[object, object] = {}
        for key, item in val.items():
            name = _scrub_text(key) if isinstance(key, str) else key
            if name in cleaned:
                raise _UnscannableJSON
...
def _scrub_json_transport(val: str, depth: int = 0) -> str:
    ...
    Text that does not parse holds no JSON payload at all, so the text scrub already
    covered everything there was to cover and the field passes through. Text that DOES
    parse but cannot be walked to the bottom -- nested past the cap, nested deeply enough
    to exhaust the parser's own recursion, or holding names that collide once scrubbed --
    has a payload this chain cannot certify, and shipping an uncertified payload is the
    hole being closed here, so the field is withheld and a marker goes out instead.
    """
