...
- **Put scratch work in `$KIROCREW_SCRATCH`, not `/tmp`.** Clones, probe scripts, logs, screenshots and pytest `--basetemp` go there: it is session-owned and reclaimed when its processes end, while `/tmp` outlives its session yet is age-deleted under live work. Sub-agents share it; `$TMPDIR` is per-process. Cross-run state belongs in neither: a directory a later run reopens is handed to you.
