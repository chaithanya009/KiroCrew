...
def remove_never_chatted(
    ...
    *,
    ...
    that recorded description (:func:`_is_fresh_sync_shape`); the bound spec,
    re-read from disk under ``agents_spec_lock`` (nested inside the config
    lock, the order every other spec writer keeps), must still carry that same
    description; and ``config.local.json`` must still not name it, read under
    the overlay's own sidecar lock, which is taken inside the base lock (the
    order every binding writer keeps) and HELD UNTIL THE BASE WRITE HAS
    COMMITTED, so an overlay writer cannot land a leaf for the name between
    the check and the delete. A row or spec that changed meanwhile -- the owner edited
    ...
    for name, candidate in candidates.items():
        ...
        # The overlay's sidecar lock is entered on this stack from inside
        # ``_mutate`` and so outlives the callback: it is released only after
        # ``update_config_locked`` has renamed the base file into place. A
        # lock released when the peek returned would leave the window the
        # peek exists to close -- an overlay writer landing a leaf for the
        # name after the check and before the base row is gone.
        with contextlib.ExitStack() as locks:
            ...
            def _mutate(
                ...
                if not _is_fresh_sync_shape(raw, kiro_agent=_bound, description=_desc):
                    return None
                # The spec's description is the one spec-derived input to the
                # shape test above; a hand edit of the spec file between
                # discovery and this delete would otherwise pass a stale value.
                # Re-read under the spec lock; a lock that cannot be taken is
                # doubt, and doubt refuses.
                try:
                    with agents_spec_lock(Path(kiro_agents_dir_path())):
                        current = _current_spec_description(_file)
                except OSError:
                    return None
                if current != _desc:
                    return None
                # Base lock first, overlay lock second -- the order the binding
                # writers keep. Read the overlay directly under it; nothing is
                # written there. An overlay that cannot be locked or read is
                # doubt, and doubt refuses.
                try:
