...
describe('New crewmate dialog', () => {
  ...
  it('a name that only differs from a crewmate\'s by case or punctuation is refused too — it would share that crewmate\'s slug', async () => {
    // The server stores "Oncall" beside "oncall" (its check is exact-name),
    // but the member thread, DM binding and projection store are keyed by
    // the slug both names share: the new crewmate would exist with a chat
    // that resolves to the old one. Refused up front, as a hint.
    await renderPage([row({ name: 'on-call', slug: 'on-call' })])
    ...
    expect(api.createKirocrewAgent).not.toHaveBeenCalled()
    expect(screen.getByTestId('crewmate-create-form')).toBeInTheDocument()
    // A genuinely different name is not caught by the slug rule.
    fireEvent.change(screen.getByLabelText('Name'), { target: { value: 'radar' } })
    fireEvent.click(screen.getByTestId('crewmate-create-submit'))
    await waitFor(() => expect(api.createKirocrewAgent).toHaveBeenCalledTimes(1))
  })
...
describe('MembersPage default member, memory and URL', () => {
  ...
  describe('below md', () => {
    ...
    it('a greeting notice does not hide the roster below md once its chat is closed — only a roster failure has no chat to show', async () => {
      ...
      // The retry is not lost: the notice keeps its record for the reopen.
      expect(screen.getByTestId('member-post-create-error')).toBeInTheDocument()
    })
  })
})
