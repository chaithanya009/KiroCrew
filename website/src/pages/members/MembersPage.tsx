...
export default function MembersPage() {
  ...
  const postCreateDismissable =
    postCreateError !== null && (postCreateError.kind === 'greeting' || members.length > 0)
  ...
  const openThread = useMutation({
    ...
    onSuccess: (r, m, seq) => {
      ...
      if (r.member !== m.name) {
        ...
        if (pendingGreets.current.delete(m.name)) setFollowUp(null)
        return
      }
      ...
      // A greeting is NOT seeded while another crewmate's follow-up notice
      // is still up: `postCreateError` holds one record, and a refused send
      // here would overwrite that one's retry. It stays parked and rides the
      // next open of this crewmate instead, once that notice is gone.
      const greet = pendingGreets.current.get(m.name)
      if (greet && !postCreateErrorRef.current) {
    ...
    onError: (error, m, seq) => {
      ...
      if (pendingGreets.current.has(m.name)) setFollowUp(null)
  ...
  return (
    ...
    <div className="flex h-full min-h-0" data-testid="members-page">
      ...
      >
        <div className={LIST_HEADER_CLS}>
          ...
          {/* Hidden while the empty roster's hero is the create door: two
              ...
              with the first row, when the hero is gone. */}
        ...
        {postCreateError && (
          ...
          >
            {/* The retry sits right after the text, not at the far edge of a
                ...
                A GREETING failure can always be dismissed: its chat is
                already open. */}
            <ErrorNotice
              message={t(
      ...
      <NewCrewmateDialog open={createOpen} onClose={() => setCreateOpen(false)} onCreated={handleCreated} existingNames={existingNames} existingSlugs={slugs} />
    </div>
  )
}
