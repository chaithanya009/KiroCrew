...
export default function NewCrewmateDialog({ open, onClose, onCreated, existingNames, existingSlugs }: {
  ...
  /**
   * The roster's slugs. A name whose slug is already here is refused too: the
   * server would store it (its check is exact-name), but the member thread,
   * the DM binding and the projection store are keyed by slug, so the new
   * crewmate's chat would resolve to the OLD one and the pair would share a
   * projection entry the page then has to withhold from both.
   */
  ...
  useEffect(() => {
    ...
    setHint(''); setError(''); setNameTaken(false); setPendingWorkspace(null)
  }, [open])
  ...
  const submit = () => {
    ...
    // Same address, different spelling ("Oncall" beside "oncall"): the server
    // would accept it, and the crewmate would then exist with a chat that can
    // never open (the thread resolves to the first-bound owner of the slug).
    // Refused here, as a hint, with what would make the name different.
      ...
      >
        ...
        >
          ...
          <Field label={t('pages.membersPage.create_name')}>
            <Input
              value={name}
              onChange={(e) => { setName(e.target.value); setHint(''); setError(''); setNameTaken(false) }}
              aria-label={t('pages.membersPage.create_name')}
              aria-invalid={hint || nameTaken ? true : undefined}
