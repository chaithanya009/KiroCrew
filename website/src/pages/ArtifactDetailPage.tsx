...
export default function ArtifactDetailPage({ popout = false }: { popout?: boolean } = {}) {
  ...
  // The anchor of the selection the open comment composer annotates (trimmed
  // for matching). A ref: written by the composer's `onOpen`, read by its
  // `onSubmit`, never rendered.
  ...
  // so an HTML/widget body the DOM toolbar cannot see into opens the same box.
  ...
  const handleComposerOpen = useCallback((text: string) => {
    ...
    else if (staged?.anchor === text) pendingAnchorRef.current = staged
    else pendingAnchorRef.current = { anchor: text }
  ...
  // Whether the open box holds unsaved text. Edit mode and a version switch
  // unmount the toolbar (and the box with it), so both ask first when this is
  // set — in the draft's own words, since "unsaved changes" would read as file
  // edits at risk. Escape / ✕ on the box ask the same question.
  ...
  // Where the draft lives between teardowns the toolbar cannot guard (a popout
  // closing, a navigation away and back). Per artifact, per passage — the same
  // record the side panel uses for this artifact, so a draft started in one
  // surface comes back in the other.
   ...
   *  ask first; a confirmed discard drops that passage's persisted copy too. */
  const guardCommentDraft = useCallback(async (proceed: () => void) => {
    ...
    proceed()
  }, [confirmDiscardDraft, composerDraftStore])
  ...
  }), [handleComposerOpen, addComment, handleComposerClose, handleComposerDraftChange, confirmDiscardDraft, composerDraftStore])
  ...
  const handleIframeSelect = useCallback((sel: IframeSelection) => {
    ...
    stagedIframeAnchorRef.current = { anchor: sel.quote, prefix: sel.prefix, suffix: sel.suffix }
  ...
  useEffect(() => { if (editing || !isCurrent) { pendingAnchorRef.current = null; stagedIframeAnchorRef.current = null; setIframeSelection(null) } }, [editing, isCurrent])
