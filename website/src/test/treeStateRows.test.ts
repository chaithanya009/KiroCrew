...
const LABELS = { empty: 'Empty folder', 'hidden-only': 'No visible files', truncated: 'Files not shown' } as const
...
describe('planTreeStateRows', () => {
  it('names one row per childless folder, with the kind the payload proves', () => {
    const plan = planTreeStateRows(
      {
        paths: ['src/a.ts'],
        directories: ['src', 'empty', 'onlyhidden', 'cut'],
        hiddenOnlyDirectories: ['onlyhidden'],
        truncatedDirectories: ['cut'],
      },
      LABELS,
    )
    expect(plan.paths).toEqual([`empty/Empty folder${M}`, `onlyhidden/No visible files${M}`, `cut/Files not shown${M}`])
