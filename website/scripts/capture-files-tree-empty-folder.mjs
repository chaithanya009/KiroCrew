 ...
 * folder (#13054): "Empty folder", "No visible files", "Files not shown: file
 * limit reached" -- and for the listing-failure notice that replaces the
 * endless shimmer when the first `/api/project/tree` request fails.
 *
 ...
 * (`truncatedDirectories`), and a populated `src/`. A dist built before this
 * fix ignores `hiddenOnlyDirectories` and paints nothing under any of them.
 *
 * Frames (`<prefix>` is the --prefix argument, e.g. `before` / `after`):
 *   <prefix>-10-folders-expanded   `_bg`, `empty`, `big` and `src` expanded
...
const TREE = {
  ...
  directories: ['_bg', 'big', 'empty', 'notes', 'src'],
  truncated: true,
...
async function main() {
  ...
  // ── Frame 10: four folders expanded ───────────────────────────────────────
  await load()
  await panel().waitFor({ state: 'visible', timeout: 20000 })
  await waitRows(r => r.some(x => x.path === '_bg/'), 'the tree to paint _bg')
  for (const p of ['_bg/', 'empty/', 'big/', 'src/']) await expand(p)
