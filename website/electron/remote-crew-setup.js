...
/**
 * Whether the gateway-failure dialog may offer to configure a remote crew.
 *
 * Exactly one state is answered by naming a crew: the local gateway is off and
 * no crew is stored for this port. Every other state already has a target -- a
 * local gateway that crashed, or a crew whose address is known and unreachable
 * -- so an address form is not its remedy. Whether the dialog can retry at all
 * is the dialog's own condition, applied where it renders the action.
 *
 * @param {{localGatewayOff?: unknown, remoteHost?: unknown}} state
 * @returns {boolean}
 */
function offersRemoteCrewSetup({ localGatewayOff = false, remoteHost = "" } = {}) {
  if (!localGatewayOff) return false;
  return String(remoteHost || "").trim() === "";
}
