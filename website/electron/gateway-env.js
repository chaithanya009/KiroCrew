 ...
 * `kiro-cli-chat` under `<resources>/backend-dist/kiro-cli/`. The backend reads
 ...
 * the backend's constant (`kiro_cli.BUNDLED_KIRO_CLI_ENTRY`).
 *
 ...
 * auto-update entirely"). Today that check is compiled for Windows only (the
 * upstream chat-cli crate's `cli/mod.rs` at v2.24.0 gates the whole block on
 * `target_os = "windows"`; macOS and Linux never start it) and Windows never
 * bundles, so nothing in this tree self-updates as it stands: the variable is a
