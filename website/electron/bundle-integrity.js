...
function describeIncompleteBundle(missing, { autoRetry = false } = {}) {
  ...
  return `Kiro Crew's bundled Python runtime is still being installed — ${detail} `
    ...
    + "still unfinished after five minutes, restart the app; if that does not "
    + "help, reinstall it.";
}
 ...
 * without Electron; the dialog only paints `message` and, on `complete`, fires
 * its own Retry action.
 *
...
function nextInstallingDialogState(missing) {
  if (!Array.isArray(missing)) {
    return { complete: false, message: describeIncompleteBundle([]) };
  }
  if (missing.length === 0) {
    return { complete: true, message: BUNDLE_COMPLETE_MESSAGE };
  }
  return { complete: false, message: describeIncompleteBundle(missing, { autoRetry: true }) };
}
