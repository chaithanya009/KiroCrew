...
function createGatewaySupervisor({
  ...
  function showGatewayErrorDialog(parentWindow, options) {
    ...
    // The other escape hatch from the client-only state: name a crew on another
    // machine instead of standing one up here. Without it this dialog offers no
    // way to reach a crew, so a launch with nothing stored can only retry the
    // same emptiness or quit.
    ...
    return new Promise((resolve) => {
      ...
      const enableButton = offerLocalStart && !noRetry
        ...
        : "";
      // The other escape hatch from the same state: name a crew on another
      // machine instead of standing one up here. Without it this dialog offers
      // no way to reach a crew, so a client-only launch with nothing stored can
      // only retry the same emptiness or quit.
      const remoteSetupButton = showRemoteSetup
        ? "<button class=\"cancel\" onclick=\"act('configure-remote')\">Add Remote Crew…</button>"
        : "";
