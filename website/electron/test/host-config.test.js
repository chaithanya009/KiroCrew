...
describe("selectLaunchPort", () => {
  ...
  describe("with the local gateway on", () => {
    ...
    // Residual, pinned at its current answer: nothing declines a plain dashboard
    // record on a scheme's default port, so a launch still lands on 80 when no
    // crew is configured there. Every per-port lookup keyed off the window URL
    // reads "" for that port, which is the classifier defect, not this decision.
    it("still takes a crew-free dashboard record on port 80", () => {
      const store = mockStore({ remoteHosts: {} });
      assert.equal(select(store, { configuredPort: 80 }).port, 80);
    });
  });
});
